...
from direct_link_based_shortest_uncovered_gurobi import (
    TNTPNetworkData,
    extract_path_for_od,
    select_ods,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--network-dir",
        default=str(Path(__file__).resolve().parents[1] / "data" / "cropped_subgraph"),
        help="Directory containing net.tntp, trips.tntp, node.tntp, and the direct MILP script.",
    )
    parser.add_argument(
        "--zone-geometries",
...
def main() -> None:
    ...
    zone_geometries = load_zone_geometries(args.zone_geometries)
    ...
    out_svg = os.path.join(network_dir, f"{args.prefix}.svg")
    out_png = os.path.join(network_dir, f"{args.prefix}.png")
    out_html = os.path.join(network_dir, f"{args.prefix}.html")
    out_summary_html = os.path.join(network_dir, f"{args.prefix}_summary.html")
    out_json = os.path.join(network_dir, f"{args.prefix}.json")
